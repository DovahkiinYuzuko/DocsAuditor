...
- **説明**:
  ...
  - `function_declaration` や `method_definition` などの関数定義、`variable_declarator` などの変数定義、さらに `interface_declaration`（インターフェース）や `type_alias_declaration`（型エイリアス）からシンボル情報を抽出して `SymbolInfo` を返します。

### `parse_python_code` (L186-196)
- **引数**:
...
### `collect_used_symbols` (L706-726)
- **引数**:
  - `node: tree_sitter::Node` - 構文ツリー of the ノード。
  - `source: &str` - ソースコード of the テキスト内容。
...
### `fallback_missing_symbols` (L624-668)
- **引数**:
  - `code_symbols: &mut Vec<SymbolInfo>` - ASTによって抽出されたソースコード内のシンボルリスト。
  - `spec_symbols: &[SymbolInfo]` - 仕様書から抽出されたシンボルリスト。
  - `code_text: &str` - ソースコードのテキスト内容。
- **戻り値**: `void`
- **説明**:
  - `spec_symbols` の中に記述されているが、`code_symbols`（ASTでの解析結果）に含まれていないシンボルを特定する。
  - 該当するシンボルについて、単語境界を考慮した正規表現（`\bシンボル名\b`）で `code_text` 全体を検索する。
  - ソースコード内にその文字列が含まれている最初の行番号（1-indexed）を `line_range` に設定した仮の `SymbolInfo`（仕様書から種類 `kind` を引き継ぐ）を生成し、`code_symbols` に追加する。これによって、ASTによる抽出漏れがあった場合でも、文字列検索によるフォールバックが働く。
  - また、フォールバックされたシンボルが仕様書で依存関係を持つ場合の誤判定を防ぐため、ソースコード全体から抽出された識別子（単語）のリスト（自分自身の名前を除く）を `used_symbols` フィールドに格納する。
...
graph TD
    ...
    fallback_missing_symbols --> SymbolInfo
...
- 新関数 `fallback_missing_symbols` の呼び出しを `main.rs` のコード解析処理の後に追加する必要があります。