...
- **フィールド**:
  ...
  - `issues_cache: Arc<Mutex<std::collections::HashMap<url::Url, Vec<crate::auditor::AuditIssue>>>>` - 各ファイルごとの最新監査エラー結果を保持するキャッシュ。Windows環境のドライブレター大文字小文字の違いによる二重登録を防ぐため、キー of Urlは `normalize_url` を用いて、URLエンコードを破壊しないように文字列置換でドライブレターのみを小文字に正規化して使用する。
...
- **説明**:
  ...
  - 対象ファイルが仕様書かコードかを判別し、対になるファイルと言語（"rust", "typescript", "python", "go", "c", "cpp", "csharp", "ruby", "swift", "kotlin", "java" など）を特定する。
...
### `collect_project_used_symbols` (L889-893)
- **引数**:
...
### `collect_used_symbols_in_dir_recursive` (L895-934)
- **引数**:
  - `dir: &Path` - 走査対象ディレクトリ。
...
### `walk_node_for_identifiers` (L936-976)
- **引数**:
...
- `normalize_url` を利用する `publish_diagnostics` や `issues_cache` のキー登録箇所。URLエンコードが破壊されなくなったため、VS Code上での警告表示およびクイックフィックスが正常に動作するようになる。
